...
func (cl *ClFile) Wstat(fid *srv.FFid, dir *ninep.Dir) error {
	return nil
}
...
func (cl *Clone) Read(fid *srv.FFid, buf []byte, offset uint64) (int, error) {
	...
	err := ncl.Add(root, name, ninep.OsUsers.Uid2User(os.Geteuid()), nil, 0666, ncl)
	if err != nil {
		return 0, &ninep.Error{"can not create file", 0}
	}
	...
	if len(buf) < len(b) {
		// cleanup
		ncl.File.Remove()
		return 0, &ninep.Error{"not enough buffer space for result", 0}
	}
...
func main() {
	...
	root = new(srv.File)
	err = root.Add(nil, "/", user, nil, ninep.DMDIR|0777, nil)
	if err != nil {
		goto error
	}

	cl = new(Clone)
	err = cl.Add(root, "clone", ninep.OsUsers.Uid2User(os.Geteuid()), nil, 0444, cl)
	if err != nil {
		goto error
	}
