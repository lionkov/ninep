...
var Eunknownfid error = &ninep.Error{"unknown fid", ninep.EINVAL}
var Enoauth error = &ninep.Error{"no authentication required", ninep.EINVAL}
var Einuse error = &ninep.Error{"fid already in use", ninep.EINVAL}
var Ebaduse error = &ninep.Error{"bad use of fid", ninep.EINVAL}
var Eopen error = &ninep.Error{"fid already opened", ninep.EINVAL}
var Enotdir error = &ninep.Error{"not a directory", ninep.ENOTDIR}
var Eperm error = &ninep.Error{"permission denied", ninep.EPERM}
var Etoolarge error = &ninep.Error{"i/o count too large", ninep.EINVAL}
var Ebadoffset error = &ninep.Error{"bad offset in directory read", ninep.EINVAL}
var Edirchange error = &ninep.Error{"cannot convert between files and directories", ninep.EINVAL}
var Enouser error = &ninep.Error{"unknown user", ninep.EINVAL}
var Enotimpl error = &ninep.Error{"not implemented", ninep.EINVAL}
...
type AuthOps interface {
	...
	// authenticated
	AuthInit(afid *Fid, aname string) (*ninep.Qid, error)
...
type Fid struct {
	...
	Omode     uint8       // Open mode (ninep.O* flags), if the fid is opened
	Type      uint8       // Fid type (ninep.QT* flags)
	Diroffset uint64      // If directory, the next valid read position
	User      ninep.User  // The Fid's user
...
func (srv *Srv) Start(ops interface{}) bool {
	...
	srv.ops = ops
	if srv.Upool == nil {
		srv.Upool = ninep.OsUsers
	}

	if srv.Msize < ninep.IOHDRSZ {
		srv.Msize = ninep.MSIZE
	}

	if srv.Log == nil {
		srv.Log = ninep.NewLogger(1024)
	}
...
func (req *Req) Process() {
	...
	if tc.Fid != ninep.NOFID && tc.Type != ninep.Tattach {
		srv.Lock()
