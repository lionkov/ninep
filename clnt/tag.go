...
func (tag *Tag) Auth(afid *Fid, user ninep.User, aname string) error {
	req := tag.reqAlloc()
	req.fid = afid
	err := ninep.PackTauth(req.Tc, afid.Fid, user.Name(), aname, uint32(user.Id()), tag.clnt.Dotu)
	if err != nil {
		return err
	}
...
func (tag *Tag) Attach(fid, afid *Fid, user ninep.User, aname string) error {
	...
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTattach(req.Tc, fid.Fid, afno, user.Name(), aname, uint32(user.Id()), tag.clnt.Dotu)
	if err != nil {
		return err
	}
...
func (tag *Tag) Open(fid *Fid, mode uint8) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTopen(req.Tc, fid.Fid, mode)
	if err != nil {
		return err
	}
...
func (tag *Tag) Create(fid *Fid, name string, perm uint32, mode uint8, ext string) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTcreate(req.Tc, fid.Fid, name, perm, mode, ext, tag.clnt.Dotu)
	if err != nil {
		return err
	}
...
func (tag *Tag) Read(fid *Fid, offset uint64, count uint32) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTread(req.Tc, fid.Fid, offset, count)
	if err != nil {
		return err
	}
...
func (tag *Tag) Write(fid *Fid, data []byte, offset uint64) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTwrite(req.Tc, fid.Fid, offset, uint32(len(data)), data)
	if err != nil {
		return err
	}
...
func (tag *Tag) Clunk(fid *Fid) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTclunk(req.Tc, fid.Fid)
	if err != nil {
		return err
	}
...
func (tag *Tag) Stat(fid *Fid) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTstat(req.Tc, fid.Fid)
	if err != nil {
		return err
	}
...
func (tag *Tag) Wstat(fid *Fid, dir *ninep.Dir) error {
	req := tag.reqAlloc()
	req.fid = fid
	err := ninep.PackTwstat(req.Tc, fid.Fid, dir, tag.clnt.Dotu)
	if err != nil {
		return err
	}
